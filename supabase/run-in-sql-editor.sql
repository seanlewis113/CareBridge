...
-- Who's Responsible (see migration 20260816130000_whos_responsible.sql)
...
CREATE INDEX IF NOT EXISTS responsibility_assignments_area_id_idx
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'responsibility_areas_select'
  ) THEN
...
-- Rx Tracker (see migration 20260822180000_prescriptions.sql)
...
CREATE INDEX IF NOT EXISTS prescription_doses_prescription_id_idx
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'prescriptions_select'
  ) THEN
